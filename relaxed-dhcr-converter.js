...
/**
 * RELAXED DHCR CSV CONVERTER
 * 
 * Converts DHCR CSV format to Supabase format with relaxed validation
 * - Address length filter: 5 → 3 characters
 * - Handles missing building numbers by using street names only
 * - More inclusive building detection
 */
...
class RelaxedDHCRCSVConverter {
    ...
    /**
     * Convert DHCR CSV to Supabase format with relaxed validation
     */
    async convertDHCRCSV(inputPath, outputPath = null) {
        console.log('🔄 RELAXED DHCR CSV CONVERTER');
        ...
        try {
            ...
            // Convert to Supabase format with relaxed validation
            const converted = this.convertToSupabaseFormatRelaxed(parsed.data, parsed.meta.fields);
    ...
    /**
     * Convert DHCR data to Supabase format with relaxed validation
     */
    convertToSupabaseFormatRelaxed(data, headers) {
        console.log('🔄 Converting to Supabase format (relaxed validation)...');
        ...
        for (const row of data) {
            try {
                const building = this.convertSingleRowRelaxed(row);
                
                if (building && building.address && building.address.length >= 3) { // RELAXED: 5 → 3 chars
    ...
    /**
     * Convert single DHCR row with relaxed validation
     */
    convertSingleRowRelaxed(row) {
        ...
        // Build address from components with RELAXED logic
        let address = '';
        
        // RELAXED: Allow addresses with just street names (no building number required)
        if (bldgNo1 && bldgNo1.toString().trim()) {
            address += bldgNo1.toString().trim();
        }
        
        if (street1 && street1.toString().trim()) {
            if (address) address += ' '; // Add space if building number exists
            address += street1.toString().trim();
        }
        
        if (suffix1 && suffix1.toString().trim()) {
        ...
        // RELAXED validation: Must have at least a street name (3+ chars) even without building number
        if (!address || address.length < 3) { // RELAXED: 5 → 3 chars
            return null;
        }
        
        // RELAXED: Accept if we have at least a street name, even without building number
        if (!street1 || street1.toString().trim().length < 2) {
            return null; // Must have at least a 2-character street name
        }
        ...
        return {
            ...
            dhcr_source: 'csv',
            confidence_score: bldgNo1 ? 95 : 85, // Lower confidence for street-only addresses
    ...
    /**
     * Normalize borough name using DHCR county codes
     */
    normalizeBoroughName(borough) {
        if (!borough) return 'manhattan';
        
        const normalized = borough.toString().trim();
        
        // DHCR County Code Mapping
        const dhcrCountyMap = {
            '60': 'bronx',        // Bronx County
            '61': 'brooklyn',     // Kings County (Brooklyn)
            '62': 'manhattan',    // New York County (Manhattan)
            '63': 'queens'        // Queens County
        };
        
        // First try DHCR county codes
        if (dhcrCountyMap[normalized]) {
            return dhcrCountyMap[normalized];
        }
        
        // Fallback to text-based mapping for other formats
        const textNormalized = normalized.toLowerCase();
        const textBoroughMap = {
            'manhattan': 'manhattan',
            'new york': 'manhattan',
            'ny': 'manhattan',
            'brooklyn': 'brooklyn',
        ...
        return textBoroughMap[textNormalized] || 'manhattan';
    }
...
function showManualConversionGuide() {
    ...
    console.log('   node relaxed-dhcr-converter.js your-dhcr-file.csv');
    console.log('');
    console.log('5. 💾 Save to database:');
    console.log('   SAVE_TO_DB=true node relaxed-dhcr-converter.js your-dhcr-file.csv');
    console.log('');
...
// Main execution
async function main() {
    const converter = new RelaxedDHCRCSVConverter();
    ...
    if (args.length === 0) {
        ...
        console.log('');
        console.log('Usage:');
        console.log('  node relaxed-dhcr-converter.js <dhcr-file.csv>           # Convert only');
        console.log('  SAVE_TO_DB=true node relaxed-dhcr-converter.js <file>    # Convert and save to DB');
        console.log('  node relaxed-dhcr-converter.js --guide                   # Show conversion guide');
        console.log('');
        showManualConversionGuide();
        return;
    }
    ...
    const outputPath = args[1] || inputPath.replace('.csv', '-relaxed-supabase.csv');
    
    try {
        const buildings = await converter.processFile(inputPath);
        
        console.log('\n🎉 RELAXED CONVERSION COMPLETE!');
        console.log('=' .repeat(50));
        ...
        if (!process.env.SAVE_TO_DB) {
            ...
            console.log(`   SAVE_TO_DB=true node relaxed-dhcr-converter.js ${inputPath}`);
        }
        
    } catch (error) {
        console.error('💥 Relaxed conversion failed:', error.message);
        console.log('');
        console.log('💡 Need help? Run:');
        console.log('   node relaxed-dhcr-converter.js --guide');
        process.exit(1);
    }
}

// Export for use in other modules
module.exports = RelaxedDHCRCSVConverter;
